...
# running SQL fetch
try:
    avg_norms = cur.execute("select avg(normprice),cards.id from prices, cards where prices.id=cards.id and cards.reserved='True' and datetime>(?) and datetime<(?) group by cards.id",(last_week,yesterday_date,))
...
try:
    ...
    for card in rl_list:
        cur.execute("insert or replace into RESERVEDCHANGE values (?,?)", (card[0],card[1]))
except:
